...
----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    EXEC Admin.Utilities.logs.di_Batch @BatchOutID OUTPUT,  @BatchOutID, 'DetailStart', @BatchDetailDescription
      ...
      SELECT 
		 ...
		 ,CASE WHEN allInputs.[PatientMobileNo] IS NULL THEN ''
		       WHEN CHARINDEX('(', allInputs.[PatientMobileNo]) = 1 AND CHARINDEX(')', allInputs.[PatientMobileNo]) = 5 THEN SUBSTRING(allInputs.[PatientMobileNo], 2, 3)
			   ELSE ''
	      END AS [PhoneAreaCityCode] 		   
		 ,CASE WHEN allInputs.[PatientMobileNo] IS NULL THEN ''
		       WHEN CHARINDEX('(', allInputs.[PatientMobileNo]) = 1 AND CHARINDEX(')', allInputs.[PatientMobileNo]) = 5 THEN REPLACE(SUBSTRING(allInputs.[PatientMobileNo], 6, LEN(allInputs.[PatientMobileNo])),'-','')
			   ELSE ''
